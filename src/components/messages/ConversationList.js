...
const ConversationList = ({ onSelectConversation, filteredConversations }) => {
  ...
  return (
    <div className="bg-white h-full flex flex-col">
      <h2 className="px-4 py-2 sm:py-3 text-sm sm:text-base font-black uppercase tracking-widest border-b text-gray-800 bg-gray-50/50 backdrop-blur-sm sticky top-0 z-10">Mensajes</h2>
        ...
        {conversationsToRender.map((conv) => {
          ...
          return (
            <li
              ...
              className={`group p-1.5 xs:p-2 hover:bg-indigo-50 rounded-xl cursor-pointer flex items-center transition-all duration-200 relative ${isUnread ? 'bg-indigo-50/30' : ''}`}
            >
              <div className="relative">
                <img
                  src={profileImage}
                  alt={displayName}
                  className={`w-8 h-8 xs:w-10 xs:h-10 rounded-full mr-2.5 xs:mr-3 object-cover border shadow-sm group-hover:scale-105 transition-transform ${isUnread ? 'border-indigo-400 ring-2 ring-indigo-100' : 'border-gray-100'}`}
                  onError={(e) => { e.target.src = defaultProfile; }}
                />
                {!conv.isGroup && isOnline && (
                  <div className="absolute bottom-0 right-3 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full shadow-sm"></div>
                )}
                {isUnread && (
                  <div className="absolute -top-1 -left-1 flex h-3 w-3">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-3 w-3 bg-indigo-600 border-[1.5px] border-white shadow-[0_0_8px_rgba(79,70,229,0.8)]"></span>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-center mb-0.5">
                  <h3 className={`truncate text-[11px] sm:text-xs uppercase tracking-wide ${isUnread ? 'font-black text-indigo-900' : 'font-bold text-gray-800'}`}>{displayName}</h3>
                  {isUnread && <span className="w-2 h-2 bg-indigo-600 rounded-full shrink-0 ml-2 shadow-sm shadow-indigo-200"></span>}
                </div>
                {!conv.isGroup && otherUser && (
                  <p className={`text-[9px] sm:text-[10px] truncate ${isUnread ? 'text-indigo-600 font-bold' : 'text-gray-500 font-semibold'}`}>{otherUser.name} {otherUser.lastName}</p>
                )}
                {conv.isGroup && (
                  <p className="text-[9px] sm:text-[10px] text-indigo-500 font-bold uppercase tracking-widest">{conv.participants.length} Miembros</p>
                )}
              </div>

              <button
                onClick={(e) => { e.stopPropagation(); handleDeleteConversation(conv._id); }}
                className="opacity-0 group-hover:opacity-100 p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all ml-1"
